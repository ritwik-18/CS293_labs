...
bool operator == (const struct employee_t &lhs, const struct employee_t &rhs) {
  //TODO: implement ==
  bool found = true;
  found = (lhs.department_name == rhs.department_name);
  if(!found) found = (lhs.id_in_department == rhs.id_in_department);
  return found; // dummy
}
...
std::strong_ordering operator <=> (const struct employee_t &lhs, 
  ...
  //TODO: implement <==>
  if(lhs.department_name < rhs.department_name) return std::strong_ordering::less;
  else if(lhs.department_name > rhs.department_name) return std::strong_ordering::greater;
  else{ 
    if(lhs.id_in_department < rhs.id_in_department) return std::strong_ordering::less;
    else if(lhs.id_in_department > rhs.id_in_department) return std::strong_ordering::greater;
